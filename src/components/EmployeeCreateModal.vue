<template>
    <v-row justify="center">
        <v-dialog v-model="employeesStore.isCreateModalOpen" persistent width="600">
            <v-form @submit.prevent ref="form">
                <v-card>
                    ...
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="blue-darken-1" variant="text" @click="employeesStore.isCreateModalOpen = false">
                            Закрыть
                        </v-btn>
...
const submit = async () => {
    ...
    if (valid) {
        employeesStore.addNewEmployee();
        employeesStore.isCreateModalOpen = false
    }
}
const InputRule = [
    (value: string) => {
